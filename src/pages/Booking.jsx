  ...
  if (done) {
    return (
      <div style={{ maxWidth: 620, margin: '60px auto 100px', padding: '0 28px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--text-body)', lineHeight: 1.6 }}>{L(OZ_DATA.bookingServices[service])} · {L(OZ_DATA.branches[branch])}<br />{t.days[day]} · {slot}</p>
    ...
    <div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '36px 28px 80px', display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 40, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          <Field label={t.s1}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {OZ_DATA.bookingServices.map((s, i) => <Tag key={s.en} selected={service === i} onClick={() => setService(i)}>{L(s)}</Tag>)}
            </div>
          </Field>
          <Field label={t.s2}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {OZ_DATA.branches.map((b, i) => (
                <button key={b.en} onClick={() => setBranch(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', textAlign: 'start', borderRadius: 'var(--radius-md)', border: `1.5px solid ${branch === i ? 'var(--pine-700)' : 'var(--border-hair)'}`, background: branch === i ? 'var(--pine-50)' : 'var(--white)', cursor: 'pointer' }}>
                  <Icon name="map-pin" size={18} color="var(--pine-700)" />
                  <span style={{ fontSize: 15, color: 'var(--text-strong)' }}>{L(b)}</span>
                </button>
              ))}
            </div>
          </Field>
        ...
        <Card padding="lg" style={{ position: 'sticky', top: 96 }}>
          ...
          {[[t.rowService, L(OZ_DATA.bookingServices[service])], [t.rowBranch, L(OZ_DATA.branches[branch])], [t.rowDate, t.days[day]], [t.rowTime, slot]].map(([k, v]) => (
          ...
          <div style={{ marginTop: 16 }}><Button variant="primary" block size="lg" onClick={() => setDone(true)}>{t.confirm}</Button></div>

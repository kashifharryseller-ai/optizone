...
await page.getByLabel('Search everything').fill('jensen')
await page.getByRole('button', { name: /Jensen Optical/ }).first().click()
await page.getByText('Products & catalog').waitFor()
const searchVal = await page.getByLabel('Search products').inputValue()
expect(searchVal.includes('Jensen'), `landed on Products pre-filtered ("${searchVal}")`)
...
console.log('\n== Content editor: English-only with auto-translation ==')
await page.locator('aside nav button', { hasText: 'Content & Homepage' }).first().click()
await page.waitForTimeout(500)
const autoNotes = await page.getByText('auto → עברית · العربية').count()
expect(autoNotes > 0, `${autoNotes} content fields show the auto-translate note (English-only entry → he/ar)`)
